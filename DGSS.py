...
# os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 代表只使用第1个gpu
...
class DynamicGhostConv(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1, act_layer=nn.SiLU, num_experts=4):
        super(DynamicGhostConv, self).__init__()
        self.oup = oup
...
class DGCS_Block(nn.Module):
    def __init__(self, inp, oup):
        super(DGCS_Block, self).__init__()
        self.in_proj = nn.Linear(inp, inp * 2)
        self.in_proj1 = nn.Linear(inp, inp // 2)
        self.conv1 = DynamicGhostConv(inp=inp, oup=oup)
        self.conv2 = DynamicGhostConv(inp=inp, oup=inp // 2)
        self.self_attention = SS2D(d_model=oup // 2)
        self.ln_1 = LN(oup)
        # self.con1 = nn.Conv2d(dim, dim, 1)

    def forward(self, x):
        x = self.conv1(x)  # B C H W
        x1 = x.clone()
        # print(x1.shape)
        x = self.ln_1(x)  # B C H W
        # print(x.shape)
        x = x.permute(0, 2, 3, 1)
        x = self.self_attention(x)  # B H W C/2
        x = x.permute(0, 3, 1, 2)
        # print(x.shape)
        x1 = self.conv2(x1)
        # print(x1.shape)
...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=4, flag=True):
        ...
        self.conv2 = nn.Conv2d(in_planes * ratio, in_planes, 1, bias=False)
        self.conv3 = nn.Conv2d(in_planes, in_planes, 1, bias=False)
        self.flag = flag
    ...
    def forward(self, x):
        ...
        out = avg_out + max_out
        out = self.conv3(self.sigmoid(out) * x)
        return out


class DGCS(nn.Module):
    def __init__(self, inp, oup):
        super(DGCS, self).__init__()
        self.in_proj = nn.Linear(inp, inp * 2)
        blocks = nn.ModuleList()
        blocks.append(DGCS_Block(inp=inp, oup=oup))
        blocks.append(DGCS_Block(inp=inp, oup=oup))
        blocks.append(DGCS_Block(inp=inp, oup=oup))
        blocks.append(DGCS_Block(inp=inp, oup=oup))
        self.proj = nn.Sequential(
            # ECA(5 * dim),
            nn.Conv2d(4 * oup // 2, oup, kernel_size=3, padding=1, groups=oup),
            nn.ReLU(),
            LN(oup)
        )
        self.convs = blocks
        self.con1 = nn.Conv2d(oup, oup, kernel_size=1)
        self.con2 = nn.Conv2d(inp, inp // 2, kernel_size=1)
        self.con3 = nn.Conv2d(oup * 2, oup, kernel_size=1)
    ...
    def forward(self, x):
        ...
        out1 = self.ca(self.proj(res)) + self.con3(res)
        out2 = self.mlp(self.ln(out1)) + out1
        out = out2 + x1
        return out
...
if __name__ == "__main__":
    ...
    model = DGCS(inp=256, oup=256).cuda()
